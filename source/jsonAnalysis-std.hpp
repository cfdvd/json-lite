...
namespace JsonAnalysis {
    ...
    template <typename... T>
    inline __attribute__((__always_inline__)) constexpr void ignore_unused(T &&...) {}
    ...
    public:
        ...
        template <typename T>
        std::optional<T> ConvertToNumber(std::string_view str) {
            T value {};
        ...
        std::pair<int, JsonData_s> Parse(std::string_view str) {
            ...
            if(str[0] == '{') {
                ...
                for(; i != strSize;) {
                    ...
                    ret.try_emplace(std::move(std::get<std::string>(key.inner)), std::move(value));
                    if(str[i] == ',') { ++i; }
                }
            ...
            if((str[0] >= '0' && str[0] <= '9') || str[0] == '-' || str[0] == '+') {
                std::cmatch what {};
                if(std::regex_search(str.begin(), str.end(), what, NumberRegex)) {
        ...
        void PrintJsonElem() {
            ...
            auto myPrint = make_overload(
                    [&](std::nullptr_t data) { ignore_unused(data); std::cout << "\tnull \n"; },
                    [&](bool data) { std::cout << std::format("\tbool, {}\n", data); },
                    [&](int data) { std::cout << std::format("\tint, {}\n", data); },
                    [&](double data) { std::cout << std::format("\tdouble, {}\n", data); },
                    [&](std::string data) { std::cout << std::format("\tstring, {}\n", data); },
                    [&](auto data) { ignore_unused(data); std::cout << "\tunknown type\n"; }
            );

            // 递归的 lambda
            auto visitor = [&](auto &func, const JsonData_s &data) ->void {
                std::visit([&](const auto &elem) ->void {
    ...
    const std::regex JsonAnalysis::NumberRegex (R"(([+-]?\d+)(.\d+)?([eE][+-]\d+)?)");
